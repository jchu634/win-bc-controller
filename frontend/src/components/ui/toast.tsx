...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed inset-x-4 top-4 z-[100] mx-auto w-auto max-w-sm outline-none sm:start-auto sm:end-4 sm:mx-0 sm:w-full",
        className,
      )}
      {...props}
    />
    ...
    <Toast
      ...
      className={
        toastItem.type === "error"
          ? "border-destructive bg-popover text-popover-foreground"
          : undefined
      }
    >
      <ToastContent>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <ToastTitle className={toastItem.type === "error" ? "text-destructive" : undefined} />
          <ToastDescription />
        </div>
        <ToastAction />
        <ToastClose />
