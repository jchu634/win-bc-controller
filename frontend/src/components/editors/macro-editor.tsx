...
export const MacroEditor = forwardRef<MacroEditorHandle, MacroEditorProps>(function MacroEditor(
  { name, onDeleted, onRenamed },
  ref,
) {
  ...
  const { macroActive, isPaused, startInline, pause, resume, cancel } = useMacroRunner();
  ...
  const dirty = savedText !== null && (value !== savedText || blockDirty);
  ...
  const save = useCallback(async (): Promise<SaveResult> => {
    ...
    setSaving(true);
    const result = await Effect.runPromise(putMacro(name, value)).catch((e): null => {
      ...
      try {
        ...
        if (confirmedName === undefined) {
          ...
          const existingName = names.find(
            (candidate) =>
              candidate !== name &&
              candidate.toLocaleLowerCase() === targetName.toLocaleLowerCase(),
          );
  ...
  }, [name, value, editorMode, blockValid, syntaxPrecheck, startInline]);
  ...
  const showBlocks = useCallback(() => {
    ...
    switch (result.kind) {
      ...
      case "unsupported": {
        ...
        setMarkers(line === null ? [] : [{ line, severity: "error", message: result.message }]);
        setError(result.message);
        return;
      }
    }
  ...
  const discardChanges = useCallback(() => {
    ...
    if (editorMode === "json" && editorHandle.current?.replaceDocument(savedText)) {
      setBlockDirty(false);
      setMarkers([]);
      setError(null);
      return;
    ...
    <section className="flex w-full flex-col gap-3 text-left">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ms-auto flex flex-wrap items-center gap-2">
          ...
          <Button
            size="sm"
            onClick={() => void save()}
            disabled={saving || !dirty || (editorMode === "blocks" && !blockValid)}
          >
            {saving ? (
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {overwriteName === null ? "Rename macro" : "Overwrite macro?"}
            </DialogTitle>
            <DialogDescription>
              {overwriteName === null
          ...
          {overwriteName === null && (
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" && newName.trim() && newName.trim() !== name)
                  void rename();
              }}
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => void remove()} disabled={deleting}>
              {deleting && <SpinnerGapIcon size={14} className="animate-spin" />}
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
